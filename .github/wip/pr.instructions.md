...
- 大きな機能は、次のいずれかの戦略で「小さい PR の列」にする。
  ...
  - スタック（依存関係を明示した複数 PR を順に積む：チームが許容する場合）
...
- ドキュメント更新が必要なら同 PR に含めるか、別 PR にするかを方針化する（チーム規約を優先）。

# レビュー・マージの品質
- CI が通る状態を維持する（既知の失敗があるなら理由と対応方針を明記する）。
- 差分が大きい場合は、なぜ大きいのか、分割できない理由、レビューの推奨順序を本文に書く。
- 可能なら「後で直す」前提の未完成状態を main に入れない（入れるなら必ず安全策を用意する）。

# してはいけないこと
- 無関係な変更を「ついで」で混ぜた PR を常態化させない。
- フォーマッタ全体適用のような大規模機械的変更を、挙動変更 PR に混ぜない。
- PR を巨大化させてレビューを諦めさせない（分割、段階導入、機能フラグを優先する）。

# PR 本文テンプレート（推奨）
以下の構造を推奨します（リポジトリに既存テンプレートがあればそれを優先）。
