...
5. CI/CD: Use `biome ci` (read-only mode) in continuous integration

### Minimal Changes Principle
Preserve existing code structure unless formatting/linting requires changes.
Use `biome check --write --changed` to target only modified files.
...
Set `recommended: true` for linter rules as baseline.
...
```
**Rule**: Never use `biome-ignore` without specific reason.
